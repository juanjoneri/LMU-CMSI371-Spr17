...
(() => {
    ...
    */
    ...
    let fillRect = (context, x, y, w, h, c1, c2, c3, c4) => {
        ...
        Modify the fillRect function in the Primitives object
        so that it interprets the first two colors (c1 and c2),
        when present, as filling the given rectangle with a radial
        gradient emanating from its center to the smaller of its two dimensions.

        - move right and down and calculate the distance to the center using pitagoras
            - si la distancia es mayor a la mitad del lado mas corto pintar con el segundo color
            - si es menor, tomar el ratio d/r to find el porcentaje corespondiente a la gradiente
            - pintar con el color correspondiente a ese valor

        For example, the radial gradient a 100×40 rectangle w
        ill have a radius of 20 pixels; the radial gradient for
        a 30×98 rectangle will have a radius of 15 pixels.

        Your new implementation may ignore the three- or four-color case: instead,
        just call the two-color case all the time, disregarding the extra color arguments.

        Hint: The overall structure of the fill does not have to change.
        It’s only the color calculation that needs modification.
        */

        let fillRectTwoColorRadialGradient = () => {
            let Yc = y + h/2; // | coordinates of the..
            let Xc = x + w/2; // | center of the circle
            let edge = w < h ? w/2 : h/2; // after the edge we just paint with the 2nd color

            for (let i = y; i < bottom; i += 1) {
                for (let j = x; j < right; j += 1) {

                    let Dsq = (i-Yc)*(i-Yc) + (j-Xc)*(j-Xc);
                    if ( Dsq > (edge * edge) ) {
                        setPixel(context, j, i, ...c2);
                    } else {
                        percent = Math.sqrt(Dsq) / edge;
                        gradientPoint = [c1[0] - (c1[0] - c2[0]) * percent,
                        c1[1] - (c1[1] - c2[1]) * percent,
                        c1[2] - (c1[2] - c2[2]) * percent];
                        setPixel(context, j, i, ...gradientPoint);
                    }
                }
            }
        ...
        if (!c1) {
            ...
            (c2[2] - c1[2]) / h];
            // fillRectTwoColors();
            fillRectTwoColorRadialGradient();
        } else {
